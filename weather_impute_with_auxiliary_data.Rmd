...
```{r, eval = FALSE}
save(scan_ts, awn_ts, amfrc1_ts, amfrc2_ts, file = 'project/weather_data/4ts_intermediate.csv')
```
...
Apply relevant transformations to some of the variables.

```{r}
simple_scan_awn_fits <- group_nest_dt(all_long, variable)
simple_scan_awn_fits[, fit := map(data, ~ lm(SCAN ~ AWN, data = .))]
```
