...
amf_rc1_raw <- fread('project/weather_data/DHSVM_Pullman/Weather Data/AMF_US-RC1_BASE-BADM_2-5/AMF_US-RC1_BASE_HH_2-5.csv', skip = 2)
amf_rc2_raw <- fread('project/weather_data/DHSVM_Pullman/Weather Data/AMF_US-RC2_BASE-BADM_2-5/AMF_US-RC2_BASE_HH_2-5.csv', skip = 2)
...
### PROCESS AMF
# Note: currently only use RC1 because of issues with RC2 timestamp.
...
amf_rc1_raw[amf_rc1_raw == -9999] <- NA

# Convert units (wind speed m/s to mph, precip mm to in)
amf_rc1_raw[, WS := convert_units(WS, 'm/s', 'mi/h')]
amf_rc1_raw[, P := convert_units(P, 'mm', 'in')]

# Convert timestamps to dates and times. Use END date. RC2 seems to have some issues.
amf_rc1_raw[, Date := as.Date(substr(as.character(TIMESTAMP_END), 1, 8), format = '%Y%m%d')]
amf_rc1_raw[, Time := as.numeric(substr(as.character(TIMESTAMP_END), 9, 10))]
amf_rc1_raw[, Minute := as.numeric(substr(as.character(TIMESTAMP_END), 11, 12))]
amf_rc1_raw[, Month := month(Date)]
amf_rc1_raw[, Day := day(Date)]

# Get only needed variables. PPFD is used for solar radiation because it does not look like SW is in there. 
amf_rc1_subset <- amf_rc1_raw[, .(Date, Time, Month, Day, Minute, TA, WS, RH, PPFD_IN, P)]

amf_vars <- c('TA', 'WS', 'RH', 'PPFD_IN', 'P')
amf_cont_vars <- c('TA', 'WS', 'RH', 'PPFD_IN')

# Get rolling mean and sum
amf_rc1_rollmean <- copy(amf_rc1_subset)
...
```{r}
SCAN_daily <- data_fivehour_filled[, .(T_min = min(`TOBS.I-1 (degC)`), T_max = max(`TOBS.I-1 (degC)`), WS = mean(`WSPDX.H-1 (mph)`), RH = mean(`RHUM.I-1 (pct)`), SR = mean(`SRADV.H-1 (watt)`), P = sum(`PRCP.H-1 (in)`)), by = .(Date, Month, Day)]

AWN_daily <- awn_rollmean[, .(T_min = min(air_temp_C), T_max = max(air_temp_C), WS = mean(WIND_SPEED_MPH), RH = mean(`RELATIVE_HUMIDITY_%`), SR = mean(SOLAR_RAD_WM2), P = sum(PRECIP_INCHES)), by = .(Date, Month, Day)]

AMFRC1_daily <- amf_rc1_rollmean[, .(T_min = min(TA), T_max = max(TA), WS = mean(WS), RH = mean(RH), SR = mean(PPFD_IN), P = sum(P)), by = .(Date, Month, Day)]
...
all_daily <- rbindlist(list(SCAN_daily_long, AWN_daily_long, AMFRC1_daily_long))
```
...
The Ameriflux is not as close as AWN but still pretty good. It looks like AMF has a warmer minimum temperature than SCAN, less wind, less humid, but maybe more precipitation. SR is probably the same but they are not in the same units thus why the red line does not line up.
