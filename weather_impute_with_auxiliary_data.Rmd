...
Now that we have the data ready, we can fit a very basic model. I think for imputation purposes it is fine to ignore covariation between the different weather variables, as well as temporal autocorrelation, as long as we can get adequate predictive power. 

## Temperature

First, for temperature, let's try AWN alone, AMF RC1 alone, AMF RC2 alone, and all three. Compare how well each of the three time series do (and all three together) when used as predictors of SCAN in the 10-fold cross-validated random forest model. All default tuning parameters are used for the random forest. If it turns out that predictions are unsatisfactory we can modify that. Parallel processing is enabled too.

```{r}
set.seed(7272)
rf_control_nobound <- trainControl(method = 'cv', number = 10, verboseIter = TRUE)

temp_awn_fit <- train(
  form = SCAN ~ AWN,
  data = all_long[variable == 'T_degC' & !is.na(SCAN) & !is.na(AWN)],
  method = 'rf',
  trControl = rf_control_nobound
)
...
```{r}
temp_pred_IS <- round(predict(temp_awn_fit), 1)
temp_pred_OS <- round(predict(temp_awn_fit, newdata = all_long[variable == 'T_degC' & !is.na(AWN)]), 1)
```
...
ws_pred_IS <- round(exp(predict(ws_awn_fit)) - 0.01, 1)
ws_pred_OS <- round(predict(ws_awn_fit, newdata = all_long[variable == 'WS_mph' & !is.na(AWN)]), 1)
```
...
rh_pred_IS <- round(100 * plogis(predict(rh_awn_fit) + 0.001), 0)
```


## Solar radiation

Again we have a non-negative value. Sometimes it can be exactly zero, but obviously otherwise always positive. We do a log transformation with a small amount added before modeling.
...
This is a difficult one because of the large number of zero values.